...
import * as contents from "../src/commands/contents.ts";
import * as similar from "../src/commands/similar.ts";
import * as answer from "../src/commands/answer.ts";
import * as research from "../src/commands/research.ts";
...
describe("search command", () => {
  ...
  test("calls search with numResults option", async () => {
    ...
    await search.search(mockClient as any, "test query", { "num-results": "10", text: false, highlights: false, summary: false });
  ...
  test("calls searchAndContents when content options are present", async () => {
    ...
    await search.search(mockClient as any, "test query", { text: true, highlights: false, summary: false });
...
describe("contents command", () => {
  ...
  test("rejects invalid URLs", async () => {
    ...
    const originalExit = process.exit;
    const exitMock = mock(() => { throw new Error("process.exit called"); }) as any;
    process.exit = exitMock;

    await expect(contents.contents(mockClient as any, ["not-a-url"], {})).rejects.toThrow("process.exit called");
  ...
  test("handles multiple URLs", async () => {
    ...
    await contents.contents(mockClient as any, ["https://example.com", "https://test.com"], {});
...
describe("research commands", () => {
  test("researchCreate creates task with instructions", async () => {
    const mockClient = {
      research: {
        ...
        pollUntilFinished: mock(() => ({ researchId: "test-id", status: "completed" })),
      },
    };
  ...
  test("researchCreate maps model correctly", async () => {
    ...
    await research.researchCreate(mockClient as any, "test instructions", { model: "fast" });
  ...
  test("researchList respects limit and cursor", async () => {
    ...
    await research.researchList(mockClient as any, { limit: "10", cursor: "test-cursor" });

    process.exit = originalExit;
    expect(mockClient.research.list).toHaveBeenCalledWith({ limit: 10, cursor: "test-cursor" });
  });
});
