...
function mockProcessExit() {
  ...
  return { originalExit, restore: () => { process.exit = originalExit; return exitCalled; } };
}

describe("E2E Tests - Real API", () => {
  beforeAll(() => {
    if (!apiKey) {
      console.warn("Skipping E2E tests: EXA_API_KEY environment variable not set");
      process.exit(0);
    }
  ...
  describe("search command", () => {
    ...
    test("performs search with content options", async () => {
      const mock = mockProcessExit();
      try {
        await search.search(client, "machine learning", { text: true, "num-results": "3" });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
  ...
  describe("contents command", () => {
    ...
    test("extracts content with text option", async () => {
      const mock = mockProcessExit();
      try {
        await contents.contents(client, ["https://example.com"], { text: true });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
  ...
  describe("similar command", () => {
    ...
    test("finds similar pages with exclude-source-domain", async () => {
      const mock = mockProcessExit();
      try {
        await similar.similar(client, "https://example.com", { "exclude-source-domain": true });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
  ...
  describe("answer command", () => {
    test("gets AI answer without streaming", async () => {
      const mock = mockProcessExit();
      try {
        await answer.answer(client, "What is machine learning?", { stream: false });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
    ...
    test("gets AI answer with streaming", async () => {
      const mock = mockProcessExit();
      try {
        await answer.answer(client, "What is quantum computing?", { stream: true });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
  ...
  describe("research commands", () => {
    ...
    test("creates research task with fast model", async () => {
      const mock = mockProcessExit();
      try {
        await research.researchCreate(client, "What is neural networks?", { model: "fast" });
        expect(true).toBe(true);
      } catch (e) {
        expect((e as Error).message).toBe("process.exit called");
      } finally {
