...
import { parseNumber, parseStringList } from "../utils/validation.ts";

export async function search(client: ExaClient, query: string, args: Record<string, unknown>) {
  try {
    const options = buildSearchOptions(args);
    
    const hasContentOptions = args.text === true || args.highlights === true || args.summary === true;
    
    const response = hasContentOptions
      ? await client.searchAndContents(query, options)
      : await client.search(query, options);
    
    console.log(format.formatSearchResults(response, { json: args.json === true }));
  } catch (error) {
