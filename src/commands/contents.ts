...
import { isValidUrl, parseNumber } from "../utils/validation.ts";

export async function contents(client: ExaClient, urls: string[], args: Record<string, unknown>) {
  try {
    for (const url of urls) {
      if (!isValidUrl(url)) {
        console.error(`Error: Invalid URL "${url}". URLs must use HTTP or HTTPS protocol.`);
        process.exit(1);
      }
    }
