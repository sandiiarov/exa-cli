...
export async function researchCreate(client: ExaClient, instructions: string, args: Record<string, unknown>) {
  try {
    ...
    if (args.model && typeof args.model === "string") {
      ...
      if (mappedModel) {
        ...
        console.error(`Warning: Invalid research model "${args.model}". Valid options are: fast, regular, pro. Using default model.`);
      }
    }
...
export async function researchStatus(client: ExaClient, researchId: string, args: Record<string, unknown>) {
  try {
...
export async function researchList(client: ExaClient, args: Record<string, unknown>) {
  try {
